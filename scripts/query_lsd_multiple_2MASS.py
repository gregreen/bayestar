...
def main():
    ...
    '''query = ("select equgal(ra, dec) as (l, b), "
             ...
             "from ucal_magsqz_noref, "
             "tmass(outer, matchedto=ucal_magsqz_noref, dmax=2.0, nmax=1)"
    ...
    if 'flux' in values.ps1_cat:
        query = (
            ...
            "where (numpy.sum(nmag_ok > 0, axis=1) >= {n_ps1_bands:d}) "
            "& (numpy.sum(nmag_ok, axis=1) >= {n_det:d}) "
            "& (numpy.sum(mean - mean_ap < 0.1, axis=1) >= {n_pointlike:d})"
            "".format(
              n_det=values.n_det,
              ps1_cat=values.ps1_cat,
              n_pointlike=values.n_pointlike,
              n_ps1_bands=values.n_PS1_bands
            ))
        ...
        query = (
            ...
            "where (numpy.sum(nmag_ok > 0, axis=1) >= {n_ps1_bands:d}) "
            "& (numpy.sum(nmag_ok, axis=1) >= {n_det:d}) "
            "& (numpy.sum(mean - mean_ap < 0.1, axis=1) >= {n_pointlike:d})".format(
              n_det=values.n_det,
              ps1_cat=values.ps1_cat,
              n_pointlike=values.n_pointlike,
              n_ps1_bands=values.n_PS1_bands
