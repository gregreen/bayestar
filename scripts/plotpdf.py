...
def main():
    ...
    parser.add_argument('-chi2', '--chi2-cut', type=float,
                              help='Cut if best chi^2/passband > X')
    ...
    # Load in chi^2/passband
    if args.chi2_cut is not None:
        with h5py.File(fname, 'r') as f:
            chi2 = f[group + '/star_chi2'][:]
        print('chi^2/passband : ')
        print(chi2)
        print('')
        idx_chi2 = (chi2 < args.chi2_cut)
        print('idx_chi2 : ')
        print(idx_chi2)
        print('')
    ...
    if args.show_pdfs:
        ...
        try:
            ...
            idx = np.ones(pdf_indiv.shape[0], dtype='bool')
            if stellar_chains:
                idx &= conv & lnZ_idx
            if args.chi2_cut is not None:
                idx &= idx_chi2
